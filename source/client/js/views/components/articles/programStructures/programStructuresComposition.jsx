...
class ProgramStructuresComposition extends Component {
	getTabMenu() {
		return map(Config.programTypes, val => (
			<TabMenuItem key={val} item={val} stores={this.props.stores} />
		));
	}
...
class TabMenuItem extends ComponentStateStore {
	...
	onClick() {
		this.props.stores.programStructureTab.set({ tab: this.props.item, tree: StructureFolding[this.props.item] });
		this.props.stores.viewport.set({
			viewport: resolve(
				this.state.viewport,
				`/program-structures/${this.props.item}`
			)
		});
	}

	isActive() {
		return this.state.tab === this.props.item ? "active tab-menu-item" : "tab-menu-item";
	}
